...
const MyProfile = () => {
  ...
  return (
    <div className=" max-w-lg flex flex-col gap-2 text-sm">
      ...
      {isEdit ? (
        <input
        className=" bg-gray-50 text-3xl font-medium
         max-w-64 mt-4"
          type="text"
          value={userData.name}
          onChange={(e) =>
        ...
        <p className=" mt-4 font-medium text-3xl text-neutral-900">{userData.name}</p>
      )}
      ...
      <div>
        {
          isEdit
          ? <button
          className=" border border-primary px-8 py-2
           rounded-lg hover:bg-primary hover:text-white transition-all duration-500
           "
